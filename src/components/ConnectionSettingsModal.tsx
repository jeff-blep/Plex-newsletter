...
export default function ConnectionSettingsModal({ isOpen, onClose, onSaved }: Props) {
  ...
  async function handleTest(kind: "plex" | "tautulli") {
    ...
    setNotice(null);
    try {
      // Server-backed “production” tests
      if (kind === "plex") {
      ...
      // Client-direct alternative (uncomment ONLY if you want browser to hit services directly)
      // if (kind === "plex") {
      //   const ok = await testConnection({ type: "plex", plexUrl, plexToken });
      //   if (!ok) throw new Error("Plex test failed");
      //   setNotice("Plex connection OK.");
      // } else {
      //   const ok = await testConnection({ type: "tautulli", tautulliUrl, tautulliApiKey });
      //   if (!ok) throw new Error("Tautulli test failed");
      //   setNotice("Tautulli connection OK.");
      // }
    } catch (e: any) {
      setError(e?.message || String(e));
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-base-300/60 backdrop-blur-sm">
      <div className="w-full max-w-2xl rounded-xl shadow-xl bg-base-100 border border-base-300">
          ...
          <section className="space-y-2">
            <div className="grid gap-2 md:grid-cols-2">
              <label className="flex flex-col">
                <span className="text-sm opacity-70">SMTP Sender Name</span>
                <input className="input input-bordered" value={smtpUser} readOnly />
              </label>
              <label className="flex flex-col">
                <span className="text-sm opacity-70">SMTP From Address</span>
