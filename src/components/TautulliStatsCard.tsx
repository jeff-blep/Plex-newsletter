...
function probeUrl(days: number) {
  return `${apiBase()}/tautulli/home?days=${encodeURIComponent(days)}`;
}
...
function plexImg(path?: string) {
  ...
  // Go through our proxy so browsers don’t hit LAN certs directly.
  return `${apiBase()}/plex/image?u=${encodeURIComponent(full)}`;
}
...
function dsTopEpisodesFromHome(json: any, limit = 5): CardItem[] {
  // Many Tautulli setups don’t include an explicit “top_episodes” in home stats.
  // We’ll try to derive episodes from any rows marked as episode.
  const rows = pickHomeRows(json, [
    "top_episodes",
    "most_watched_episodes",
    "most_watched_tv",
    "top_tv",
  ]);
  const arr = (rows || [])
...
function computeTotals(json: any): Totals {
  try {
    // Plays: sum the “top_users” block counts as a decent approximation; fall back to summing rows.
    const usersRows = pickHomeRows(json, ["top_users"]);
    let plays = 0;
    if (usersRows.length) {
      plays = usersRows.reduce((s: number, r: any) => s + Number(r?.total_plays || r?.plays || 0), 0);
    } else {
      // fallback: sum visible movie/show rows
      const allBlocks = Array.isArray(json?.home) ? json.home : [];
    ...
    // Duration: keep max per rating_key to avoid double counting across blocks
    ...
    return (
      <div className={common}>
        <svg viewBox="0 0 24 24" className="w-6 h-6 opacity-80">
          <path
            className={text}
            d="M16 13c-1.1 0-2 .9-2 2 0 1.1.9 2 2 2 .6 0 1.2-.3 1.6-.7.4-.4.7-1 .7-1.6 0-1.1-.9-2-2-2Zm-5.2-2.7c.5-1 1.4-1.6 2.4-1.6.4 0 .8.1 1.1.2-.2.6-.5 1.2-.9 1.7-.6.8-1.3 1.2-1.9 1.2-.4 0-.7-.1-1-.3-.1-.4-.1-.8.3-1.2ZM12 5c-4 0-7 3-7 7v6c0 1.7 1.3 3 3 3h8c1.7 0 3-1.3 3-3v-6c0-4-3-7-7-7Z"
          />
        </svg>
      </div>
    );
    ...
    return (
      <div className={common}>
        <svg viewBox="0 0 24 24" className="w-6 h-6 opacity-80">
          <path
            className={text}
            d="M17.6 8.1l1.3-2.2a.5.5 0 10-.9-.5l-1.4 2.4A7.6 7.6 0 0012 7c-1.6 0-3 .4-4.6.8L6 5.4a.5.5 0 10-.9.5l1.3 2.2A6.7 6.7 0 004 14v4a1 1 0 001 1h1a1 1 0 001-1v-3h1v3a1 1 0 001 1h6a1 1 0 001-1v-3h1v3a1 1 0 001 1h1a1 1 0 001-1v-4a6.7 6.7 0 00-2.4-5.9zM9 9.5a.75.75 0 110-1.5.75.75 0 010 1.5zm6 0a.75.75 0 110-1.5.75.75 0 010 1.5z"
          />
        </svg>
      </div>
    );
    ...
    return (
      <div className={common}>
        <svg viewBox="0 0 24 24" className="w-6 h-6 opacity-80">
          <path
            className={text}
            d="M16 2H8a3 3 0 00-3 3v14a3 3 0 003 3h8a3 3 0 003-3V5a3 3 0 00-3-3zm-4 18a1.5 1.5 0 110-3 1.5 1.5 0 010 3z"
          />
        </svg>
      </div>
    );
    ...
    return (
      <div className={common}>
        <svg viewBox="0 0 24 24" className="w-6 h-6 opacity-80">
          ...
          <path
            className={text}
            d="M12 2a10 10 0 019.3 6H12a4 4 0 00-3.5 2L5 6.1A10 10 0 0112 2zm0 20a10 10 0 01-8.7-5L7.8 9.8A4 4 0 0012 14h9.3A10 10 0 0112 22z"
          />
        </svg>
      </div>
    );
...
export default function TautulliStatsCard({ days = 7 }: { days?: number }) {
  ...
  return (
    <div className="card bg-base-200 shadow-md p-4 rounded-md">
      ...
      {error && (
        <div className="alert alert-error text-sm break-words">
          <div className="font-semibold mb-1">Fetch error</div>
          <div className="mb-1">{error}</div>
          <div className="opacity-80">
            URL tried: <code>{probeUrl(days)}</code>{" "}
            <a className="link" href={probeUrl(days)} target="_blank" rel="noreferrer">
              open in new tab
            </a>
          </div>
        </div>
      ...
      {data && !loading && !error && (
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 pt-4">
            <ListCard title="Most Watched Movies" items={dsTopMovies(data, 5)} />
            <ListCard title="Most Watched TV Shows" items={dsTopShows(data, 5)} />
            <ListCard title="Most Watched Episodes" items={dsTopEpisodesFromHome(data, 5)} />
            <ListCard title="Most Used Platforms" items={dsTopPlatforms(data, 5)} unit="plays" />
          </div>
        </div>
      )}
    </div>
