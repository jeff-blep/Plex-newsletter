...
export default function SettingsPage() {
  ...
  async function save(partial: any) {
    ...
    try {
      ...
      setConfig(updated);
      (window as any).toast?.success?.("Saved ✓") ?? alert("Saved ✓");
    } catch (e: any) {
      setError(e?.message || String(e));
      (window as any).toast?.error?.("Save failed: " + (e?.message || e)) ?? alert("Save failed: " + (e?.message || e));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleRunNow() {
    try {
      ...
      (window as any).toast?.success?.("Sent: " + JSON.stringify(res.sent)) ?? alert("Sent: " + JSON.stringify(res.sent));
    } catch (e: any) {
      (window as any).toast?.error?.("Run failed: " + (e?.message || e)) ?? alert("Run failed: " + (e?.message || e));
    }
  }

  function setScheduleMode(mode: ScheduleMode) {
    if (mode === "custom") {
      const cron = prompt("Enter CRON expression (min hour day month weekday)", config?.schedule?.cron || "0 9 * * 1");
  ...
  function toggleInclude(k: IncludeKeys, val: boolean) { save({ include: { [k]: val } }); }
  ...
  const scheduleMode: ScheduleMode = (config.schedule?.mode || "daily") as ScheduleMode;
    ...
    <div className="min-h-screen bg-base-200">
      ...
      <div className="navbar bg-base-100 shadow">
        ...
        <div className="flex-none gap-2 pr-2">
          <button className="btn btn-accent" onClick={handleRunNow} disabled={saving}>Send Newsletter Now</button>
        </div>
      </div>
        ...
        <div className="stats shadow bg-base-100">
          <div className="stat">
            ...
            <div className="stat-desc">
              {scheduleMode === "custom" ? (config.schedule?.cron || "") : (scheduleMode === "daily" ? "Every day at 09:00" : "Mondays at 09:00")}
            </div>
          </div>
          <div className="stat">
            <div className="stat-title">Lookback</div>
            <div className="stat-value text-lg">{config.lookbackDays || 7}d</div>
